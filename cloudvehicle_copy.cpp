...
int first_spot_cluster[65], temp_spot, failcount = 0, jobcomplete = 0, migration_count = 0, failure_migration_count = 0, assign_count = 0, count_job_complete = 0;
void R1();
void R2();
void R3();
void R4();
void Generate_Array_for_cluster();
...
int timefile(){
	...
	cout << "Would you like to create another random number?(0/1)" << endl;
	cin >> again;
	cout << endl;
...
{
	public:
	int center_id, region_id, cluster_id, spot_id, spot_flag, car_num_in_spot;
...
class jobs_assigned
{
	...
	int car_id_data1, car_id_data2, car_id_data3, time_taken_to_replicate_within_cluster, time_taken_to_replicate_within_region, success_migration;
	bool complete, assign;
	double VM_size, job_duration, time_taken_to_store_data, remaining_time_job;
	string data_replicated;
	double data_produced;
	// success_migration will 2 for successful migration and 3 for failure
	...
	void data_replication_time_SameCluster();
	void data_replication_time_DiffCluster_SameRegion();
...
//Initially the whole parking lot is full. This function is used to print all the information related to cars
void car :: show_car_info(ofstream& outputfile)                  
		{                                                                 
			outputfile<<"---------------------------------- Information about Car ----------------------------------" << endl;
...
jobs_assigned :: jobs_assigned()
{
	...
	time_taken_to_replicate_within_cluster = 0;
}
...
{
	data_produced = rand()%(1000-500)+500;
}
...
void jobs_assigned :: data_replication_time_SameCluster()
{
	time_taken_to_replicate_within_cluster = (data_produced/2)/600;
	// Data replication happens during the execution of a job. According to our assumption, it takes 10MB/sec link speed to transfer the data to a car located in the same cluster and hence 600MB/minute is considered for calculation.
}

void jobs_assigned :: data_replication_time_DiffCluster_SameRegion()
{
	time_taken_to_replicate_within_region = (data_produced/2)/600 + (data_produced/2)/60000 +  (data_produced/2)/600;
	// This calculates the time taken to replicate the data while the job is being carried out and the car is located in a different cluster but lpcated in the same region.
}

void jobs_assigned :: job_duration_calculation()
{
	job_duration = (VM_size * 0.1) + time_taken_to_store_data + time_taken_to_replicate_within_cluster + time_taken_to_replicate_within_region;
...
// Cars are let into different parking lots based on regions, centers, clusters and a particular parking lot which is available. This is the initialization phase.
// Every region is divided into 4 equal centers and then into clusters. Each cluster contains 40 parking lots 
// There are 16 center and 64 clusters and 2560 parking lots in total	
void R1() {
...
void Generate_Array_for_cluster()
{
	...
	for( i = 0; i <65; i++)
	{
		first_spot_cluster[i] = 0;
	}
	first_spot_cluster[1] = 1;
	
	for( i = 0; i < 65; i++)
	{
		first_spot_cluster[i] = first_spot_cluster[i-1] + 40;
	}
}

// This function finds the car that stays for longest duration within a cluster. 
int max_dep(int spot, int parking_spot_of_job) 
{
	...
	return cars[spotnumber].car_id; // Returns the car ID of the car that has maximum departure time in the cluster.
}
...
int random_cluster_id_generation( int car_cid, int car_spot_id, int car_cluster_number)
{
	int random_num = 1, rand_num_cluster = 0, car_num, spot_num, cid_of_car;
	
	Generate_Array_for_cluster();
	...
	for(cid_of_car = 1; cid_of_car <= 16; cid_of_car++)
	{
		if(cid_of_car == car_cid)
		{
			...
			do
			{
				rand_num_cluster = rand()%4 + random_num;
			} while(rand_num_cluster == car_cluster_number);
			
			car_num = max_dep(first_spot_cluster[rand_num_cluster], car_spot_id);
		}
...
int main()
{
	...
	for( i = 1; i < 7861; i++)
	{
		...
		jobs[i].data_replication_time_SameCluster();
		jobs[i].data_replication_time_DiffCluster_SameRegion();
	...
	// When we reach the last cluster, we have to reset
	if(spot_num[1].cluster_id > 64)
	{
		spot_num[1].center_id = 1;
		spot_num[1].cluster_id = 1;
	...
	for ( i = 1; i < 7861; i++)
	{
		...
		if( cars[i].car_arr_time <= End_time )
			{
				total_cars++;
	...
	{
		spot_num[i].show_parking_info(outSpot,i);
		spot_num[i].show_parking_info(outSpot,i);
	}
	
	Time_track time;
	cout <<" Time: " << time.time_minutes << endl;
	
	for( i = 1; i < 1441; i++)
	{
		time.increment_time();
		cout << endl << " Time: " << time.time_minutes;
		...
		for ( int m = 1; m < 2561; m++ )
		{
			...
			{
				int this_car_id, this_spot_id, this_cluster_id;
				
				this_spot_id = cars[this_car_id].parking_spot;
				this_cluster_id = spot_num[this_spot_id].cluster_id;
				this_car_id = jobs[m].car_id;
				
				Testoutput1 << " Current job's Spot ID: " << this_spot_id << endl << "Current job's Cluster ID: " << this_cluster_id << endl;
				
				for ( n = 1; n < 7861; n++ )
				{
					int this_spot_id_car, this_center_id_car, this_cluster_id_car, diff_cluster_car_id;
					
					this_spot_id_car = cars[n].parking_spot;
					this_center_id_car = spot_num[this_spot_id].center_id;
					this_cluster_id_car = spot_num[this_spot_id].cluster_id;
					
					
					if ( this_cluster_id_car == this_cluster_id && spot_num[this_spot_id_car].spot_flag != -1)
					{
						jobs[m].car_id_data1 =  max_dep(this_spot_id_car, this_spot_id);
						
						Testoutput1 << " Car ID: " << this_car_id << endl << "Spot ID: " << this_spot_id << endl << "Cluster_id : " << this_cluster_id << endl << "(Belongs to same cluster) Data replication to the car: " << jobs[m].car_id_data1 << "Job duration: " << jobs[m].remaining_time_job << endl;
						
						random_cluster_id_generation( this_center_id_car, this_spot_id, this_cluster_id_car );
						
						Testoutput1 << " Car ID: " << this_car_id << endl << "Spot ID: " << this_spot_id << endl << "Cluster_id : " << this_cluster_id << endl << " Cluster ID: " << this_cluster_id << endl << "Job duration: " << jobs[m].remaining_time_job << endl;
