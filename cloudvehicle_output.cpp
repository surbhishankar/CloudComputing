...
void parking_lot_info :: show_parking_info(ofstream& outputfile, int i)
{
	...
	outputfile<<"AccessPoint id of the parking spot :- "<<access_point_id<<endl;
...
{
	job_duration = (VM_size * 0.1) + time_taken_to_store_data + time_taken_to_replicate_within_access_point + time_taken_to_replicate_within_region;
	...
	for( i = 1; i < 1441; i++)
	{
		...
		for ( int m = 1; m < 2561; m++ )
		{
			...
			if ( temp_job_time == temp_remaining_job)
			{
				...
				//Testoutput1 << " Current job's Spot ID: " << this_spot_id << endl << "Current job's AccessPoint ID: " << this_access_point_id << endl;
				...
				for ( n = 1; n < 5001; n++ )
				{
					...
					if ( this_access_point_id_car == this_access_point_id && spot_num[this_spot_id_car].spot_flag != -1)
					{
						...
						Testoutput1 << " Car ID: " << this_car_id << endl << "Spot ID: " << this_spot_id << endl << "AccessPoint_id : " << this_access_point_id << endl << " AccessPoint ID : " << this_access_point_id << endl << "Job duration: " << jobs[m].remaining_time_job << endl;
