...
int arrival_migration(int time)
{
	int i, car_number = -1, temp_stay_time = -1;
	
	for(i = 1; i <= 7560; i++)
	{
		if(cars[i].car_arr_time > cars[i].car_dep_time && cars[i].car_dep_time > time)
		{
			if( cars[i].occupied != true && cars[i].parking_spot != -1)
			{
...
int main()
{
	...
	for( i = 1; i <= 1440; i++)
	{
		...
		for ( int j = 1; j < 7561; j++)
		{
			...
			{
				temp_car_id_migrate = arrival_migration(i);
